...
export const City = () => {
  ...
  // ---------- store ----------
  const cityData = useSelector((state) => state.citySlice.info);
  ...
  useEffect(() => {
    let options = {
      filters: [],
      includeProperties: "Province,Province.Country",
    };
    dispatch(getCities(options));
  }, [isReload]);
  useEffect(() => {
    if (cityData) {
      const values = cityData.data?.map((item, index) => ({
        rowId: index + 1,
        id: item.id,
        title: item.title,
        province: item.province.title,
        country: item.province.country.title
      }));
      setData(values);
    }
  }, [cityData]);
  ...
  const deleteHandler = (id) => {
    ...
    axios
      .delete(api.SettingsApi.deleteCity + id, { headers })
      .then((res) => {
