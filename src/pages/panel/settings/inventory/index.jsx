...
export const Inventory = () => {
  ...
  const inventoryData = useSelector(
    (state) => state.inventorySlice.info
  );
  ...
  const cols = [
    ...
    {
      name: t("table.col.inventory_number"),
      selector: (row) => 
      <p className="dark:text-dark_custom-full-white">
        {row.inventoryNumber}
      </p>
  ...
  useEffect(() => {
    if (inventoryData) {
      const values = inventoryData.data?.map((item, index) => ({
        rowId: index + 1,
        id: item.id,
        model: item.model,
        brand: item.brand,
  ...
  const deleteHandler = (id) => {
    ...
    axios
      .delete(api.SettingsApi.deleteInventory + id, { headers })
      .then((res) => {
