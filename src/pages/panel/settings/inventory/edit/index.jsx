...
export const EditInventory = () => {
  ...
  const editInfo = useSelector((state) => state.inventorySlice.editInfo);
  ...
  const onSubmit = (values) => {
    dispatch(editInventory(urlParams.id, values, navigate));
  };
  ...
  useEffect(() => {
    if (editInfo) {
      setData({ ...editInfo });
      setInventoryTypeItem({ value: editInfo.inventoryType?.id, label: editInfo.inventoryType?.title });
    }
  }, [editInfo]);
  useEffect(() => {
