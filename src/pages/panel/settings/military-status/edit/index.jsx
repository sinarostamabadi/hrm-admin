...
export const EditMilitaryStatus = ({ onCloseModal, isReloadPage, editId }) => {
  // ---------- store ----------
  const editInfo = useSelector((state) => state.militaryStatusSlice.editInfo);
  ...
  const onSubmit = (values) => {
    dispatch(
      editMilitaryStatus(editId, values, (status) => reloadPageHandler(status))
    );
  };

  // ---------- lifeCycle ----------
  useEffect(() => {
    dispatch(getByIdMilitaryStatus(editId));
  }, []);
  useEffect(() => {
  ...
  return (
    <>
      <div className="flex items-center justify-between p-4 border-b border-custom-gray-light dark:bg-dark_custom-average-black">
        <h4 className="text-18 font-bold dark:text-dark_custom-full-white">{t("page_title.edit_military_status")}</h4>
