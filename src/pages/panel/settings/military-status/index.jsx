...
import { api } from "../../../../api";
import { CreateMilitaryStatus } from "./create";
import { EditMilitaryStatus } from "./edit";
...
export const MilitaryStatus = () => {
  ...
  // ---------- store ----------
  const militaryStatusData = useSelector(
    (state) => state.militaryStatusSlice.info
  );
  const isLoading = useSelector((state) => state.loadingSlice.isLoading);
console.log(militaryStatusData);
  // ---------- hooks ----------
  ...
  useEffect(() => {
    if (militaryStatusData.length) {
      const values = militaryStatusData.map((item, index) => ({
        rowId: index + 1,
        id: item.id,
        title: item.title,
      }));
  ...
  const deleteHandler = (id) => {
    ...
    axios
      .delete(api.SettingsApi.deleteMilitaryStatus + id, { headers })
      .then((res) => {
