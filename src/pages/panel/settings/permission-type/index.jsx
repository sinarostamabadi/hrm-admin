...
import { getPermissionTypes } from "../../../../redux/actions/settings/permission-type";
...
import { api } from "../../../../api";
import { CreatePermissionType } from "./create";
import { EditPermissionType } from "./edit";
...
export const PermissionType = () => {
  ...
  // ---------- store ----------
  const permissionTypeData = useSelector(
    (state) => state.permissionTypeSlice.info
  );
  ...
  useEffect(() => {
    if (permissionTypeData) {
      const values = permissionTypeData.data?.map((item, index) => ({
        rowId: index + 1,
        id: item.id,
        title: item.title,
      }));
  ...
  const deleteHandler = (id) => {
    ...
    axios
      .delete(api.SettingsApi.deletePermissionType + id, { headers })
      .then((res) => {
