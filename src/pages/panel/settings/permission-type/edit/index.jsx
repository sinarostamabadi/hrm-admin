...
import {
  getByIdPermissionType,
  editPermissionType,
} from "../../../../../redux/actions/settings/permission-type";
import * as Yup from "yup";

export const EditPermissionType = ({ onCloseModal, isReloadPage, editId }) => {
  // ---------- store ----------
  const editInfo = useSelector((state) => state.permissionTypeSlice.editInfo);
  ...
  const onSubmit = (values) => {
    dispatch(
      editPermissionType(editId, values, (status) => reloadPageHandler(status))
    );
  };

  // ---------- lifeCycle ----------
  useEffect(() => {
    dispatch(getByIdPermissionType(editId));
  }, []);
  useEffect(() => {
    ...
    <>
      <div className="flex items-center justify-between p-4 border-b border-custom-gray-light dark:bg-dark_custom-average-black">
        <h4 className="text-18 font-bold dark:text-dark_custom-full-white">
          {t("page_title.edit_permission_type")}
        </h4>
