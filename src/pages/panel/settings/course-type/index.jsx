...
import { getCourseTypes } from "../../../../redux/actions/settings/course-type";
...
import { api } from "../../../../api";
import { CreateCourseType } from "./create";
import { EditCourseType } from "./edit";
...
export const CourseType = () => {
  ...
  // ---------- store ----------
  const courseTypeData = useSelector((state) => state.courseTypeSlice.info);
  ...
  useEffect(() => {
    if (courseTypeData) {
      const values = courseTypeData.data?.map((item, index) => ({
        rowId: index + 1,
        id: item.id,
        title: item.title,
      }));
  ...
  const deleteHandler = (id) => {
    ...
    axios
      .delete(api.SettingsApi.deleteCourseType + id, { headers })
      .then((res) => {
