...
import {
  getByIdCourseType,
  editCourseType,
} from "../../../../../redux/actions/settings/course-type";
import * as Yup from "yup";

export const EditCourseType = ({ onCloseModal, isReloadPage, editId }) => {
  // ---------- store ----------
  const editInfo = useSelector((state) => state.courseTypeSlice.editInfo);
  ...
  const onSubmit = (values) => {
    dispatch(
      editCourseType(editId, values, (status) => reloadPageHandler(status))
    );
  };

  // ---------- lifeCycle ----------
  useEffect(() => {
    dispatch(getByIdCourseType(editId));
  }, []);
  useEffect(() => {
