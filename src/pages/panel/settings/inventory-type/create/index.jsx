...
import { createInventoryType } from "../../../../../redux/actions/settings/inventory-type";
import * as Yup from "yup";

export const CreateInventoryType = ({ onCloseModal, isReloadPage }) => {
  // ---------- store ----------
  ...
  const onSubmit = (values) => {
    dispatch(createInventoryType(values, (status) => reloadPageHandler(status)));
  };
  ...
  return (
    <>
      <div className="flex items-center justify-between p-4 border-b border-custom-gray-light dark:bg-dark_custom-average-black">
        <h4 className="text-18 font-bold dark:text-dark_custom-full-white">{t("page_title.create_inventory_type")}</h4>
