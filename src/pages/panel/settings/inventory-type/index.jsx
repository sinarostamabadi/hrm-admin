...
import { api } from "../../../../api";
import { CreateInventoryType } from "./create";
import { EditInventoryType } from "./edit";
...
export const InventoryType = () => {
  ...
  useEffect(() => {
    if (inventoryTypeData) {
      const values = inventoryTypeData.data?.map((item, index) => ({
        rowId: index + 1,
        id: item.id,
        title: item.title,
      }));
  ...
  const deleteHandler = (id) => {
    ...
    axios
      .delete(api.SettingsApi.deleteInventoryType + id, { headers })
      .then((res) => {
