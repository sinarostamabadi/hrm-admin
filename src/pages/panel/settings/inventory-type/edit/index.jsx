...
import {
  getByIdInventoryType,
  editInventoryType,
} from "../../../../../redux/actions/settings/inventory-type";
import * as Yup from "yup";

export const EditInventoryType = ({ onCloseModal, isReloadPage, editId }) => {
  // ---------- store ----------
  const editInfo = useSelector((state) => state.inventoryTypeSlice.editInfo);
  ...
  const onSubmit = (values) => {
    dispatch(
      editInventoryType(editId, values, (status) => reloadPageHandler(status))
    );
  };

  // ---------- lifeCycle ----------
  useEffect(() => {
    dispatch(getByIdInventoryType(editId));
  }, []);
  useEffect(() => {
    setData({
      id: editId,
  ...
  return (
    <>
      <div className="flex items-center justify-between p-4 border-b border-custom-gray-light dark:bg-dark_custom-average-black">
        <h4 className="text-18 font-bold dark:text-dark_custom-full-white">{t("page_title.edit_inventory_type")}</h4>
