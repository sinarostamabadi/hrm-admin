...
import { api } from "../../../../api";
import { CreateEmployeeTitle } from "./create";
import { EditEmployeeTitle } from "./edit";
...
export const EmployeeTitle = () => {
  ...
  // ---------- store ----------
  const employeeTitleData = useSelector((state) => state.employeeTitleSlice.info);
  ...
  useEffect(() => {
    if (employeeTitleData.length) {
      const values = employeeTitleData.map((item, index) => ({
        rowId: index + 1,
        id: item.id,
        title: item.title,
        target: item.target,
  ...
  const deleteHandler = (id) => {
    ...
    axios
      .delete(api.SettingsApi.deleteEmployeeTitle + id, { headers })
      .then((res) => {
