...
export const EditEmployeeTitle = ({ onCloseModal, isReloadPage, editId }) => {
  // ---------- state ----------
  ...
  // ---------- store ----------
  const editInfo = useSelector((state) => state.employeeTitleSlice.editInfo);
  ...
  const onSubmit = (values) => {
    dispatch(
      editEmployeeTitle(editId, values, (status) => reloadPageHandler(status))
    );
  };

  // ---------- lifeCycle ----------
  useEffect(() => {
    dispatch(getByIdEmployeeTitle(editId));
  }, []);
  useEffect(() => {
    setData({
      id: editId,
  ...
  return (
    <>
      <div className="flex items-center justify-between p-4 border-b border-custom-gray-light dark:bg-dark_custom-average-black">
        <h4 className="text-18 font-bold dark:text-dark_custom-full-white">{t("page_title.edit_employee_title")}</h4>
