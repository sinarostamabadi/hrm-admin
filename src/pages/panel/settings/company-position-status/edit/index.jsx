...
import {
  getByIdCompanyPositionStatus,
  editCompanyPositionStatus,
} from "../../../../../redux/actions/settings/company-position-status";
import * as Yup from "yup";

export const EditCompanyPositionStatus = ({ onCloseModal, isReloadPage, editId }) => {
  // ---------- store ----------
  const editInfo = useSelector((state) => state.companyPositionStatusSlice.editInfo);
  ...
  const onSubmit = (values) => {
    dispatch(
      editCompanyPositionStatus(editId, values, (status) => reloadPageHandler(status))
    );
  };

  // ---------- lifeCycle ----------
  useEffect(() => {
    dispatch(getByIdCompanyPositionStatus(editId));
  }, []);
  useEffect(() => {
    setData({ id: editId, tenantId: editInfo.tenantId, title: editInfo.title });
  }, [editInfo]);
    ...
    <>
      <div className="flex items-center justify-between p-4 border-b border-custom-gray-light dark:bg-dark_custom-average-black">
        <h4 className="text-18 font-bold dark:text-dark_custom-full-white">
          {t("page_title.edit_company_position_status")}
        </h4>
