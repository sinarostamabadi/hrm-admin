...
import { api } from "../../../../api";
import { CreateCompanyPositionStatus } from "./create";
import { EditCompanyPositionStatus } from "./edit";

export const CompanyPositionStatus = () => {
  // ---------- state ----------
  ...
  // ---------- store ----------
  const companyPositionStatusData = useSelector((state) => state.companyPositionStatusSlice.info);
  ...
  useEffect(() => {
    if (companyPositionStatusData) {
      const values = companyPositionStatusData.data?.map((item, index) => ({
        rowId: index + 1,
        id: item.id,
        title: item.title,
      }));
  ...
  const deleteHandler = (id) => {
    ...
    axios
      .delete(api.SettingsApi.deleteCompanyPositionStatus + id, { headers })
      .then((res) => {
          ...
          <div className="flex justify-between items-center">
            <div className="flex flex-col gap-y-1">
              <h4 className="text-19 text-custom-dark font-bold dark:text-dark_custom-full-white">
                {t("page_title.company_position_status")}
              </h4>
              <h4 className="text-14 flex gap-x-2 text-custom-gray-muted dark:text-dark_custom-light-white">
                ...
                <span className="text-custom-gray-200 font-bold dark:text-dark_custom-light-white">
                  {companyPositionStatusData.count}
                </span>
              </h4>
            </div>
            <div>
