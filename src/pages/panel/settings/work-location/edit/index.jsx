...
export const EditWorkLocation = ({ onCloseModal, isReloadPage, editId }) => {
  // ---------- store ----------
  const editInfo = useSelector((state) => state.workLocationSlice.editInfo);
  ...
  const onSubmit = (values) => {
    dispatch(
      editWorkLocation(editId, values, (status) => reloadPageHandler(status))
    );
  };

  // ---------- lifeCycle ----------
  useEffect(() => {
    dispatch(getByIdWorkLocation(editId));
  }, []);
  useEffect(() => {
    setData({
      id: editId,
  ...
  return (
    <>
      <div className="flex items-center justify-between p-4 border-b border-custom-gray-light dark:bg-dark_custom-average-black">
        <h4 className="text-18 font-bold dark:text-dark_custom-full-white">{t("page_title.edit_work_location")}</h4>
