...
import { api } from "../../../../api";
import { CreateWorkLocation } from "./create";
import { EditWorkLocation } from "./edit";
...
export const WorkLocation = () => {
  ...
  // ---------- store ----------
  const workLocationData = useSelector((state) => state.workLocationSlice.info);
  ...
  useEffect(() => {
    if (workLocationData.length) {
      const values = workLocationData.map((item, index) => ({
        rowId: index + 1,
        id: item.id,
        title: item.title,
  ...
  const deleteHandler = (id) => {
    ...
    axios
      .delete(api.SettingsApi.deleteWorkLocation + id, { headers })
      .then((res) => {
