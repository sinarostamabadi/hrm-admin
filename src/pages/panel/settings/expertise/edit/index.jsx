...
import {
  getByIdExpertise,
  editExpertise,
} from "../../../../../redux/actions/settings/expertise";
import * as Yup from "yup";

export const EditExpertise = ({ onCloseModal, isReloadPage, editId }) => {
  // ---------- store ----------
  const editInfo = useSelector((state) => state.expertiseSlice.editInfo);
  ...
  const onSubmit = (values) => {
    dispatch(
      editExpertise(editId, values, (status) => reloadPageHandler(status))
    );
  };

  // ---------- lifeCycle ----------
  useEffect(() => {
    dispatch(getByIdExpertise(editId));
  }, []);
  useEffect(() => {
