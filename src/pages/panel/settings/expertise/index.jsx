...
export const Expertise = () => {
  ...
  // ---------- store ----------
  const expertiseData = useSelector((state) => state.expertiseSlice.info);
  ...
  useEffect(() => {
    if (expertiseData) {
      const values = expertiseData.data?.map((item, index) => ({
        rowId: index + 1,
        id: item.id,
        title: item.title,
      }));
  ...
  const deleteHandler = (id) => {
    ...
    axios
      .delete(api.SettingsApi.deleteExpertise + id, { headers })
      .then((res) => {
