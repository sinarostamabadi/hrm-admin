...
export const Currency = () => {
  ...
  const actions = [
    ...
    {
      type: "delete",
      api: api.SettingsApi.deleteCurrency,
    },
  ];
  ...
  useEffect(() => {
    if (currencyData) {
      const values = currencyData.data?.map((item, index) => ({
        rowId: index + 1,
        id: item.id,
        title: item.title,
        symbol: item.symbol,
  ...
  const deleteHandler = (id) => {
    ...
    axios
      .delete(api.SettingsApi.deleteCurrency + id, { headers })
      .then((res) => {
