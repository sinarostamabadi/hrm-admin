...
import { getRolePermissions } from "../../../../redux/actions/settings/role-permission";
...
export const RolePermission = () => {
  ...
  // ---------- store ----------
  const rolePermissionData = useSelector(
    (state) => state.rolePermissionSlice.info
  );
  ...
  useEffect(() => {
    if (roleData) {
      const values = roleData.data?.map((item, index) => ({
        rowId: index + 1,
        id: item.roleId,
        title: item.title,
  ...
  const deleteHandler = (id) => {
    ...
    axios
      .delete(api.SettingsApi.deleteRole + id, { headers })
      .then((res) => {
