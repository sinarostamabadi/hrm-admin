...
import { api } from "../../../../api";
import { CreateWorkType } from "./create";
import { EditWorkType } from "./edit";
...
export const WorkType = () => {
  ...
  // ---------- store ----------
  const workTypeData = useSelector((state) => state.workTypeSlice.info);
  ...
  useEffect(() => {
    if (workTypeData) {
      const values = workTypeData.data?.map((item, index) => ({
        rowId: index + 1,
        id: item.id,
        title: item.title,
      }));
  ...
  const deleteHandler = (id) => {
    ...
    axios
      .delete(api.SettingsApi.deleteWorkType + id, { headers })
      .then((res) => {
