...
import {
  getByIdWorkType,
  editWorkType,
} from "../../../../../redux/actions/settings/work-type";
import * as Yup from "yup";

export const EditWorkType = ({ onCloseModal, isReloadPage, editId }) => {
  // ---------- store ----------
  const editInfo = useSelector((state) => state.workTypeSlice.editInfo);
  ...
  const onSubmit = (values) => {
    dispatch(
      editWorkType(editId, values, (status) => reloadPageHandler(status))
    );
  };

  // ---------- lifeCycle ----------
  useEffect(() => {
    dispatch(getByIdWorkType(editId));
  }, []);
  useEffect(() => {
