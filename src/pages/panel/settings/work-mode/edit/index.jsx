...
import {
  getByIdWorkMode,
  editWorkMode,
} from "../../../../../redux/actions/settings/work-mode";
import * as Yup from "yup";

export const EditWorkMode = ({ onCloseModal, isReloadPage, editId }) => {
  // ---------- store ----------
  const editInfo = useSelector((state) => state.workModeSlice.editInfo);
  ...
  const onSubmit = (values) => {
    dispatch(
      editWorkMode(editId, values, (status) => reloadPageHandler(status))
    );
  };

  // ---------- lifeCycle ----------
  useEffect(() => {
    dispatch(getByIdWorkMode(editId));
  }, []);
  useEffect(() => {
    setData({ id: editId, title: editInfo.title });
  }, [editInfo]);
  ...
  return (
    <>
      <div className="flex items-center justify-between p-4 border-b border-custom-gray-light dark:bg-dark_custom-average-black">
        <h4 className="text-18 font-bold dark:text-dark_custom-full-white">{t("page_title.edit_work_mode")}</h4>
