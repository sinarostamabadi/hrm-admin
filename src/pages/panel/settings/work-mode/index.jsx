...
import { api } from "../../../../api";
import { CreateWorkMode } from "./create";
import { EditWorkMode } from "./edit";
...
export const WorkMode = () => {
  ...
  // ---------- store ----------
  const workModeData = useSelector((state) => state.workModeSlice.info);
  ...
  useEffect(() => {
    if (workModeData) {
      const values = workModeData.data?.map((item, index) => ({
        rowId: index + 1,
        id: item.id,
        title: item.title,
      }));
  ...
  const deleteHandler = (id) => {
    ...
    axios
      .delete(api.SettingsApi.deleteWorkMode + id, { headers })
      .then((res) => {
