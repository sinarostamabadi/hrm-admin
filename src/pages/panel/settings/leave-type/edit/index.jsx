...
import {
  getByIdLeaveType,
  editLeaveType,
} from "../../../../../redux/actions/settings/leave-type";
import * as Yup from "yup";

export const EditLeaveType = ({ onCloseModal, isReloadPage, editId }) => {
  // ---------- store ----------
  const editInfo = useSelector((state) => state.leaveTypeSlice.editInfo);
  ...
  const onSubmit = (values) => {
    dispatch(
      editLeaveType(editId, values, (status) => reloadPageHandler(status))
    );
  };

  // ---------- lifeCycle ----------
  useEffect(() => {
    dispatch(getByIdLeaveType(editId));
  }, []);
  useEffect(() => {
    setData({
      id: editId,
