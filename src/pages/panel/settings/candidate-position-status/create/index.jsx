...
import { createCandidatePositionStatus } from "../../../../../redux/actions/settings/candidate-position-status";
import * as Yup from "yup";
import { getDataFromJwtToken } from "../../../../../helpers/get-data-from-jwt";

export const CreateCandidatePositionStatus = ({ onCloseModal, isReloadPage, parentId = null }) => {
  // ---------- store ----------
  ...
  const onSubmit = (values) => {
    dispatch(createCandidatePositionStatus(values, (status) => reloadPageHandler(status)));
  };
  ...
  return (
    <>
      <div className="flex items-center justify-between p-4 border-b border-custom-gray-light">
        <h4 className="text-18 font-bold">{t("page_title.create_candidate_position_status")}</h4>
