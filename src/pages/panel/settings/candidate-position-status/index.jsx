...
import { api } from "../../../../api";
import { CreateCandidatePositionStatus } from "./create";
import { EditCandidatePositionStatus } from "./edit";

export const CandidatePositionStatus = () => {
  // ---------- state ----------
  ...
  // ---------- store ----------
  const candidatePositionStatusData = useSelector(
    (state) => state.candidatePositionStatusSlice.info
  );
  ...
  // ---------- lifeCycle ----------
  useEffect(() => {
    dispatch(getAllCandidatePositionStatus({}));
  }, [isReload]);
  ...
  const deleteHandler = (id) => {
    ...
    axios
      .delete(api.SettingsApi.deleteCandidatePositionStatus + id, { headers })
      .then((res) => {
  ...
  return (
    <>
      <Modal state={openModal} onCloseModal={closeModalHandler}>
        {modalType === "create" ? (
          <CreateCandidatePositionStatus
            onCloseModal={closeModalHandler}
            isReloadPage={() => setIsReload((currState) => !currState)}
            parentId={itemId}
          />
        ) : (
          <EditCandidatePositionStatus
            onCloseModal={closeModalHandler}
            isReloadPage={() => setIsReload((currState) => !currState)}
            editId={itemId}
          />
      ...
      <div>
        <div className="flex flex-col gap-y-8 w-full bg-white p-6 rounded-lg">
          <div className="flex justify-between items-center">
            <div className="flex flex-col gap-y-1">
              <h4 className="text-19 text-custom-dark font-bold">
                {t("page_title.candidate_position_status")}
              </h4>
              <h4 className="text-14 flex gap-x-2 text-custom-gray-muted">
                {t("table.result")}
                <span className="text-custom-gray-200 font-bold">
                  {candidatePositionStatusData.count}
                </span>
              </h4>
            </div>
            <div>
          ...
          {isLoading && !candidatePositionStatusData.count ? (
            <ClipLoader color="#FE6601" className="w-full mx-auto" />
          ) : (
            <div>
              <ul className="flex flex-col gap-y-5">
                {candidatePositionStatusData.count ?
                  candidatePositionStatusData.data.map(
                    (candidatePosition, index) =>
                      !candidatePosition.parentId && (
                        <li className="w-2/6" key={candidatePosition.id}>
                          <div className="flex items-center justify-between">
                            ...
                            <div className="flex items-center gap-x-2">
                              {actions.map((action) => (
                                <>
                                  {action.type === "delete" && (
                                    <div className="w-6 h-6 rounded-md bg-custom-gray-light flex items-center justify-center cursor-pointer" onClick={() => deleteHandler(candidatePosition.id)}>
                                      <svg
                                        width="17"
                                        height="17"
                                        viewBox="0 0 16 16"
                          ...
                          {!!candidatePosition.inverseParent.length && (
                            <div
                              className={`ml-8 ${
                            ...
                            >
                              <ul className="flex flex-col gap-y-6">
                                {candidatePosition.inverseParent.map(
                                  (subCandidate) => (
                                    <li className="w-full" key={subCandidate.id}>
                                      <div className="flex items-center justify-between">
                                        ...
                                        <div className="flex items-center gap-x-2">
                                          {actions.map((action) => (
                                            <>
                                              {action.type === "delete" && (
                                                <div className="w-6 h-6 rounded-md bg-custom-gray-light flex items-center justify-center cursor-pointer" onClick={() => deleteHandler(subCandidate.id)}>
                                                  <svg
                                                    width="17"
                                                    height="17"
                                                    viewBox="0 0 16 16"
                                              ...
                                              {action.type === "edit" && (
                                                <div className="w-6 h-6 rounded-md bg-custom-gray-light flex items-center justify-center cursor-pointer" onClick={() => openModalHandler("edit", subCandidate.id)}>
                                                  <svg
                                                    width="17"
                                                    height="17"
                                                    viewBox="0 0 16 16"
