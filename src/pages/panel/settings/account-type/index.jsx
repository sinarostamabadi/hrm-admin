...
import { getAccountTypes } from "../../../../redux/actions/settings/account-type";
...
import { api } from "../../../../api";
import { CreateAccountType } from "./create";
...
export const AccountType = () => {
  ...
  // ---------- store ----------
  const accountTypeData = useSelector((state) => state.accountTypeSlice.info);
  ...
  useEffect(() => {
    if (accountTypeData) {
      const values = accountTypeData.data?.map((item, index) => ({
        rowId: index + 1,
        id: item.id,
  ...
  const deleteHandler = (id) => {
    ...
    axios
      .delete(api.SettingsApi.deleteAccountType + id, { headers })
      .then((res) => {
