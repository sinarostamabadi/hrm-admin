...
import { api } from "../../../../api";
import { CreateContractType } from "./create";
...
export const ContractType = () => {
  ...
  // ---------- store ----------
  const contractTypeData = useSelector((state) => state.contractTypeSlice.info);
  ...
  useEffect(() => {
    if (contractTypeData.length) {
      const values = contractTypeData.map((item, index) => ({
        rowId: index + 1,
        id: item.id,
        title: item.title,
      }));
  ...
  const deleteHandler = (id) => {
    ...
    axios
      .delete(api.SettingsApi.deleteContractType + id, { headers })
      .then((res) => {
