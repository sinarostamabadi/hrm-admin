...
export const EditContractType = ({ onCloseModal, isReloadPage, editId }) => {
  // ---------- store ----------
  const editInfo = useSelector((state) => state.contractTypeSlice.editInfo);
  ...
  const onSubmit = (values) => {
    dispatch(
      editContractType(editId, values, (status) => reloadPageHandler(status))
    );
  };

  // ---------- lifeCycle ----------
  useEffect(() => {
    dispatch(getByIdContractType(editId));
  }, []);
  useEffect(() => {
    ...
    <>
      <div className="flex items-center justify-between p-4 border-b border-custom-gray-light dark:bg-dark_custom-average-black">
        <h4 className="text-18 font-bold dark:text-dark_custom-full-white">
          {t("page_title.edit_contract_type")}
        </h4>
