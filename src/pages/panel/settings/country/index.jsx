...
export const Country = () => {
  ...
  useEffect(() => {
    if (countryData) {
      const values = countryData.data?.map((item, index) => ({
        rowId: index + 1,
        id: item.id,
        title: item.title,
  ...
  const deleteHandler = (id) => {
    ...
    axios
      .delete(api.SettingsApi.deleteCountry + id, { headers })
      .then((res) => {
