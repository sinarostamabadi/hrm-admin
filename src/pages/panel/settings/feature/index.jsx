...
export const Feature = () => {
  ...
  // ---------- store ----------
  const featureData = useSelector((state) => state.featureSlice.info);
  ...
  useEffect(() => {
    if (featureData) {
      const values = featureData.data?.map((item, index) => ({
        rowId: index + 1,
        id: item.id,
        title: item.title,
      }));
  ...
  const deleteHandler = (id) => {
    ...
    axios
      .delete(api.SettingsApi.deleteFeature + id, { headers })
      .then((res) => {
