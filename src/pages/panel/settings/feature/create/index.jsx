...
import { createFeature } from "../../../../../redux/actions/settings/feature";
import * as Yup from "yup";

export const CreateFeature = ({ onCloseModal, isReloadPage }) => {
  // ---------- store ----------
  ...
  const onSubmit = (values) => {
    dispatch(createFeature(values, (status) => reloadPageHandler(status)));
  };
  ...
  return (
    <>
      <div className="flex items-center justify-between p-4 border-b border-custom-gray-light dark:bg-dark_custom-average-black">
        <h4 className="text-18 font-bold dark:text-dark_custom-full-white">{t("page_title.create_feature")}</h4>
