...
import {
  getByIdFeature,
  editFeature,
} from "../../../../../redux/actions/settings/feature";
import * as Yup from "yup";

export const EditFeature = ({ onCloseModal, isReloadPage, editId }) => {
  // ---------- store ----------
  const editInfo = useSelector((state) => state.featureSlice.editInfo);
  ...
  const onSubmit = (values) => {
    dispatch(
      editFeature(editId, values, (status) => reloadPageHandler(status))
    );
  };

  // ---------- lifeCycle ----------
  useEffect(() => {
    dispatch(getByIdFeature(editId));
  }, []);
  useEffect(() => {
  ...
  return (
    <>
      <div className="flex items-center justify-between p-4 border-b border-custom-gray-light dark:bg-dark_custom-average-black">
        <h4 className="text-18 font-bold dark:text-dark_custom-full-white">{t("page_title.edit_feature")}</h4>
