...
import { endLoading, startLoading } from "../../../redux/reducers/ui/loading";
...
export const CompanyPosition = () => {
  ...
  // ---------- store ----------
  const { info: companyPositionData, loading } = useSelector(
  ...
  const cols = [
    ...
    {
      name: t("table.col.position_detail"),
      selector: (row) => row.positionDetail,
    },
    {
      name: t("table.col.salary_range"),
      ...
      cell: (row) => (
        <div className="group relative flex items-center justify-center gap-x-2">
          <Link
            ...
            to={`/companyPosition/edit/${row.id}`}
          >
            <svg
              width="17"
              height="17"
      ...
      filters: [
        {
          property: "CompanyId",
          operation: 5,
          values: [`${state?.companyId ? state.companyId : navigate("/company")}`],
        },
      ],
      orderBy: "",
      includeProperties: "CompanyPositionExpertides,CompanyPositionFeatures",
    };
    dispatch(getCompanyPositions(params));
  }, [isReload]);
  useEffect(() => {
    if (companyPositionData) {
      const values = companyPositionData.data?.map((item, index) => ({
        rowId: index + 1,
        id: item.id,
        title: item.title,
        description: item.description,
        positionDetail: item.positionDetial,
  ...
  const deleteHandler = (id) => {
    ...
    axios
      .delete(api.CompanyPositionApi.deleteCompanyPosition + id, { headers })
      .then((res) => {
        ...
        <div className="flex justify-between items-center dark:bg-dark_custom-light-black">
          ...
          <div>
            <Button
              ...
              classes="px-4"
              onClick={() =>
                navigate("/companyPosition/create", {
                  state: {
