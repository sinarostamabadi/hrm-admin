...
import { api } from "../../../api";
import { CreateCompanyProject } from "./create";
import { EditCompanyProject } from "./edit";
...
export const CompanyProject = () => {
  ...
  // ---------- store ----------
  const { info: companyProjectData, loading } = useSelector(
    (state) => state.companyProjectSlice
  );
  ...
  useEffect(() => {
    ...
    dispatch(getCompanyProjects(params));
  }, [isReload]);
  useEffect(() => {
    if (companyProjectData) {
      const values = companyProjectData.data?.map((item, index) => ({
        rowId: index + 1,
        id: item.id,
        title: item.title,
  ...
  const deleteHandler = (id) => {
    ...
    axios
      .delete(api.CompanyProjectApi.deleteCompanyProject + id, { headers })
      .then((res) => {
